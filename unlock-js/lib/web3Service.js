...
exports.default = exports.TransactionType = exports.Constants = exports.keyId = void 0;
...
var _unlockAbi = require('unlock-abi-0');

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

const keyId = (lock, owner) => [lock, owner].join('-');
...
class Web3Service extends _events.default {
  constructor({
    readOnlyProvider,
    unlockAddress,
    blockTime,
    ...
    super();
    this.web3 = new _web.default(readOnlyProvider);
    this.unlockAddress = unlockAddress;
    this.blockTime = blockTime;
    this.requiredConfirmations = requiredConfirmations; // Transactions create events which we use here to build the state.
    ...
    this.eventsHandlers = {
      ...
      PriceChanged: (transactionHash, contractAddress, blockNumber, {
        keyPrice
      }) => {
        this.emit('transaction.updated', transactionHash, {
          lock: contractAddress
        ...
        return this.emit('lock.updated', contractAddress, {
          asOf: blockNumber,
          keyPrice: _web3Utils.default.fromWei(keyPrice, 'ether')
        });
      },
    ...
    this.inputsHandlers = {
      createLock: async (transactionHash, contractAddress, params) => {
        ...
        this.emit('lock.updated', newLockAddress, {
          ...
          keyPrice: _web3Utils.default.fromWei(params._keyPrice, 'ether'),
          // Must be expressed in Eth!
  ...
  async generateLockAddress() {
    let transactionCount = await this.web3.eth.getTransactionCount(this.unlockAddress);
    return _web3Utils.default.toChecksumAddress((0, _ethereumjsUtil.bufferToHex)((0, _ethereumjsUtil.generateAddress)(this.unlockAddress, transactionCount)));
  }
  /**
  ...
  getTransactionType(contract, data) {
    ...
    if (contract.contractName === 'PublicLock' && method.name === 'updateKeyPrice') {
  ...
  _getPastTransactionsForContract(contract, eventNames, filter) {
    return contract.getPastEvents(eventNames, {
      fromBlock: 0,
      // TODO start only when the smart contract was deployed?
      toBlock: 'latest',
      filter
    }, (error, events = []) => {
  ...
  getPastLockCreationsTransactionsForUser(address) {
    const unlock = new this.web3.eth.Contract(_unlockAbi.Unlock.abi, this.unlockAddress);
  ...
  getPastLockTransactions(lockAddress) {
    const lockContract = new this.web3.eth.Contract(_unlockAbi.PublicLock.abi, lockAddress);
  ...
  getAddressBalance(address) {
    return this.web3.eth.getBalance(address).then(balance => {
      return _web3Utils.default.fromWei(balance, 'ether');
    }).catch(error => {
  ...
  emitContractEvent(transactionHash, contractAddress, blockNumber, name, params) {
  ...
  parseTransactionLogsFromReceipt(transactionHash, contract, transactionReceipt) {
    ...
    transactionReceipt.logs.forEach(log => {
      ...
      abiEvents.forEach(event => {
        ...
        const decoded = this.web3.eth.abi.decodeLog(event.inputs, log.data, log.topics.slice(1));
        const args = event.inputs.reduce((args, input) => {
          args[input.name] = decoded[input.name];
          return args;
        }, {});
        this.emitContractEvent(transactionHash, log.address, transactionReceipt.blockNumber, event.name, args);
      });
    });
  }
  /**
  ...
  _getSubmittedTransaction(transactionHash, blockNumber, defaults) {
    this._watchTransaction(transactionHash); // If we have default values for the transaction (passed by the walletService)


    if (defaults) {
      const contract = this.unlockAddress === _web3Utils.default.toChecksumAddress(defaults.to) ? _unlockAbi.Unlock : _unlockAbi.PublicLock;
      return this.parseTransactionFromInput(transactionHash, contract, defaults.input, defaults.to);
    }
  ...
  _getPendingTransaction(blockTransaction) {
    ...
    const contract = this.unlockAddress === _web3Utils.default.toChecksumAddress(blockTransaction.to) ? _unlockAbi.Unlock : _unlockAbi.PublicLock;
    return this.parseTransactionFromInput(blockTransaction.hash, contract, blockTransaction.input, blockTransaction.to);
  }
  /**
  ...
  getTransaction(transactionHash, defaults) {
    return Promise.all([this.web3.eth.getBlockNumber(), this.web3.eth.getTransaction(transactionHash)]).then(([blockNumber, blockTransaction]) => {
      // If the block transaction is missing the transacion has been submitted but not
      // received by all nodes
      if (!blockTransaction) {
        return this._getSubmittedTransaction(transactionHash, blockNumber, defaults);
      ...
      const contract = this.unlockAddress === _web3Utils.default.toChecksumAddress(blockTransaction.to) ? _unlockAbi.Unlock : _unlockAbi.PublicLock;
      const transactionType = this.getTransactionType(contract, blockTransaction.input); // Let's watch for more confirmations if needed

      if (blockNumber - blockTransaction.blockNumber < this.requiredConfirmations) {
      ...
      return this.web3.eth.getTransactionReceipt(transactionHash).then(transactionReceipt => {
        if (transactionReceipt) {
          ...
          return this.parseTransactionLogsFromReceipt(transactionHash, contract, transactionReceipt);
        }
      });
    });
  }
  ...
  getLock(address) {
    const contract = new this.web3.eth.Contract(_unlockAbi.PublicLock.abi, address);
    const attributes = {
      keyPrice: x => _web3Utils.default.fromWei(x, 'ether'),
    ...
    constantPromises.push(this.web3.eth.getBlockNumber().then(blockNumber => {
  ...
  getKeyByLockForOwner(lock, owner) {
    const lockContract = new this.web3.eth.Contract(_unlockAbi.PublicLock.abi, lock);
    return this._getKeyByLockForOwner(lockContract, owner).then(([expiration, data]) => {
  ...
  _getKeyByLockForOwner(lockContract, owner) {
    return new Promise(resolve => {
      const getKeyExpirationPromise = lockContract.methods.keyExpirationTimestampFor(owner).call();
      const getKeyDataPromise = lockContract.methods.keyDataFor(owner).call();
      Promise.all([getKeyExpirationPromise, getKeyDataPromise]).then(([expiration, data]) => {
  ...
  _packageKeyholderInfo(lock, lockContract, ownerAddress) {
    return this._getKeyByLockForOwner(lockContract, ownerAddress).then(([expiration, data]) => {
      return {
  ...
  _genKeyOwnersFromLockContractIterative(lock, lockContract, page, byPage) {
    ...
    return new Promise(resolve => {
      let keyPromises = Array.from(Array(byPage).keys()).map(n => {
        return lockContract.methods.owners(n + startIndex).call().then(ownerAddress => {
  ...
  _genKeyOwnersFromLockContract(lock, lockContract, page, byPage) {
    return new Promise((resolve, reject) => {
      lockContract.methods.getOwnersByPage(page, byPage).call().then(ownerAddresses => {
  ...
  getKeysForLockOnPage(lock, page, byPage) {
    ...
    this._genKeyOwnersFromLockContract(lock, lockContract, page, byPage).then(keyPromises => {
      if (keyPromises.length == 0) {
        this._genKeyOwnersFromLockContractIterative(lock, lockContract, page, byPage).then(keyPromises => this._emitKeyOwners(lock, page, keyPromises));
      } else {
