...
// File @openzeppelin/contracts-ethereum-package/contracts/introspection/ERC165.sol@v2.4.0
...
// File @openzeppelin/contracts-ethereum-package/contracts/token/ERC20/IERC20.sol@v2.4.0
...
// File @openzeppelin/contracts-ethereum-package/contracts/math/SafeMath.sol@v2.4.0
...
// File @openzeppelin/contracts-ethereum-package/contracts/token/ERC20/SafeERC20.sol@v2.4.0
...
contract MixinFunds is Ownable {
   ...
   */
  function _chargeAtLeast(uint _price) internal returns (uint) {
    if (_price > 0) {
 ...
 */
contract MixinDisableAndDestroy is Ownable, MixinFunds {
  ...
  function _initializeMixinDisableAndDestroy() internal {
    isAlive = true;
  }
  ...
  function destroyLock() external onlyOwner {
    ...
    _transfer(
      tokenAddress,
      msg.sender,
      getBalance(tokenAddress, address(this))
    );
...
// File @openzeppelin/contracts-ethereum-package/contracts/access/Roles.sol@v2.4.0
...
contract MixinKeys is Ownable, MixinLockCore {
  ...
  function getTokenIdFor(
    address _account
  ) public view hasValidKey(_account) returns (uint) {
 ...
 * separates logically groupings of code to ease readability.
 */
contract MixinApproval is MixinDisableAndDestroy, MixinKeys {
  ...
  function approve(
    address _approved,
    uint _tokenId
  ) public onlyIfAlive onlyKeyOwnerOrApproved(_tokenId) {
...
interface IUnlockEventHooks {
   ...
   */
  function keyCancel(address operator, address to, uint256 refund) external;
}

// File @openzeppelin/contracts/introspection/IERC1820Registry.sol@v2.4.0
 ...
 */
contract MixinEventHooks is MixinLockCore {
  IERC1820Registry public constant erc1820 =
    IERC1820Registry(0x1820a4B7618BdE71Dce8cdc73aAB6C95905faD24);

  // `keccak256("IUnlockEventHooks_keySold")`
  bytes32 public constant keySoldInterfaceId =
    0x4d99da10ff5120f726d35edd8dbd417bbe55d90453b8432acd284e650ee2c6f0;

  // `keccak256("IUnlockEventHooks_keyCancel")`
  bytes32 public constant keyCancelInterfaceId =
  ...
  function _onKeySold(
    ...
    if (implementer != address(0)) {
      IUnlockEventHooks(implementer).keySold(
        msg.sender,
        _to,
        _referrer,
        _pricePaid,
   ...
   */
  function _onKeyCancel(address _to, uint256 _refund) internal {
    address implementer = erc1820.getInterfaceImplementer(
      beneficiary,
 ...
 * separates logically groupings of code to ease readability.
 */
contract MixinGrantKeys is Ownable, MixinKeys {
  /**
   * Allows the Lock owner to give a collection of users a key with no charge.
   * Each key may be assigned a different expiration date.
   */
  ...
  MixinKeys
{
  ...
  function tokenURI(
    ...
    return URI.strConcat(address(this).address2Str(), "/", _tokenId.uint2Str());
  }
}
  ...
  MixinEventHooks
{
  ...
  function purchase(
    ...
    if (toKey.expirationTimestamp >= block.timestamp) {
      ...
      toKey.expirationTimestamp = toKey.expirationTimestamp.add(
        expirationDuration
      );
    } else {
...
// File @openzeppelin/contracts-ethereum-package/contracts/cryptography/ECDSA.sol@v2.4.0
...
contract MixinSignatures {
  ...
  modifier consumeOffchainApproval(
    ...
    require(
      ecrecover(ECDSA.toEthSignedMessageHash(_hash), _v, _r, _s) == _keyOwner,
      "INVALID_SIGNATURE"
    );
  ...
  function invalidateOffchainApproval(uint _nextAvailableNonce) external {
    require(
      _nextAvailableNonce > keyOwnerToNonce[msg.sender],
      "NONCE_ALREADY_USED"
    );
  ...
  MixinEventHooks
{
  ...
  function fullRefund(
    address _keyOwner,
    uint amount
  ) external onlyOwner hasValidKey(_keyOwner) {
...
// File @openzeppelin/contracts-ethereum-package/contracts/token/ERC721/IERC721Receiver.sol@v2.4.0
...
contract MixinTransfer is MixinFunds, MixinLockCore, MixinKeys, MixinApproval {
  ...
  function transferFrom(
    ...
    address _recipient,
    uint _tokenId
  ) public onlyIfAlive hasValidKey(_from) onlyKeyOwnerOrApproved(_tokenId) {
    require(
      transferFeeBasisPoints < BASIS_POINTS_DEN,
      "KEY_TRANSFERS_DISABLED"
    );
   ...
   * Allow the Lock owner to change the transfer fee.
   */
  function updateTransferFee(uint _transferFeeBasisPoints) external onlyOwner {
