...
// File @openzeppelin/upgrades/contracts/Initializable.sol@v2.7.1
...
interface IUnlock {
   ...
   */
  function configUnlock(string calldata _symbol, string calldata _URI) external;
  ...
  MixinKeys
{
  ...
  function getTransferFee(
    address _keyOwner,
    uint _time
  ) public view hasValidKey(_keyOwner) returns (uint) {
    Key storage key = keyByOwner[_keyOwner];
    uint timeToTransfer;
    uint fee;
