...
// File hardlydifficult-eth/contracts/protocols/Uniswap/IUniswapOracle.sol@v1.1.1
    ...
    if (_value > 0) {
      ...
      if (_referrer != address(0)) {
        ...
        if (address(udtOracle) != address(0)) {
          ...
          uint tokensToMint = (estimatedGasForPurchase * tx.gasprice).mul(
            125 * 10 ** 18
          ) /
            100 /
            udtPrice;
          // or tokensToMint is capped by percent growth
          uint maxTokens = IMintableERC20(udt).totalSupply().mul(valueInETH) /
            2 /
            grossNetworkProduct;
