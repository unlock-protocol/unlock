...
// File hardlydifficult-eth/contracts/protocols/Uniswap/IUniswapOracle.sol@v1.1.2
...
contract Unlock is IUnlock, Initializable, Ownable {
  ...
  function createLock(
    ...
    address payable newLock = address(
      uint160(publicLockAddress.createClone2(salt))
    );
    ...
    if (_value > 0) {
      ...
      if (_referrer != address(0)) {
        ...
        if (address(udtOracle) != address(0)) {
          ...
          uint tokensToDistribute = (estimatedGasForPurchase * tx.gasprice).mul(
            125 * 10 ** 18
          ) /
            100 /
            udtPrice;
          ...
          if (chainId > 1) {
            ...
            maxTokens =
              IMintableERC20(udt).balanceOf(address(this)).mul(valueInETH) /
              (2 + (2 * valueInETH) / grossNetworkProduct) /
              grossNetworkProduct;
          } else {
            // Mainnet: we mint new token using log curve
            maxTokens =
              IMintableERC20(udt).totalSupply().mul(valueInETH) /
              2 /
              grossNetworkProduct;
          }
