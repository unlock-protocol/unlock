...
const walletMiddlewareCreator = config => {
  return function walletMiddleware({ getState, dispatch }) {
    ...
    walletService.on('account.changed', account => {
      ...
      if (config.isServer) {
        // don't do any account setting on the server
        return
      }
...
export default walletMiddlewareCreator
