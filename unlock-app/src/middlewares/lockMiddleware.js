  ...
  return function(next) {
    return function(action) {
      if (!web3Service.ready) {
        ...
        return web3Service
          ...
          .then(([networkId, account]) => {
            ...
            // We refresh keys
            Object.values(getState().keys).forEach(key =>
              web3Service.refreshKey(key)
            )
          })
          .catch(() => {
      ...
      if (action.type === LOAD_ACCOUNT) {
        web3Service.loadAccount(action.privateKey).then(account => {
          return dispatch(setAccount(account))
        })
      } else if (action.type === CREATE_ACCOUNT) {
        web3Service.createAccount().then(account => {
          return dispatch(setAccount(account))
        })
        ...
        // Create a lock
        web3Service
          .createLock(action.lock, (transaction, lock) => {
        ...
        web3Service
          .purchaseKey(action.key, account, lock, transaction => {
            dispatch(addTransaction(transaction))
          })
          .then(key => {
        ...
        web3Service
          .refreshTransaction(action.transaction)
          .then(transaction => {
        ...
        const account = getState().network.account
        web3Service
          .withdrawFromLock(action.lock, account)
          .then(lock => {
            return Promise.all([
