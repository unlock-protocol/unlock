...
describe('TwitterTags', () => {
  ...
  it('should render twitter tags based on default values', () => {
    ...
    expect(tags.find('meta[name=\'twitter:title\']').props().content).toBe(pageTitle())
    expect(tags.find('meta[name=\'twitter:description\']').props().content).toBe(PAGE_DESCRIPTION)
    expect(tags.find('meta[name=\'twitter:image\']').props().content).toBe(PAGE_DEFAULT_IMAGE)
  })
  ...
  it('should render twitter tags based on custom values', () => {
    ...
    const tags = shallow(<TwitterTags title={title} description={description} image={image} />)
    expect(tags.find('meta[name=\'twitter:title\']').props().content).toBe(title)
    expect(tags.find('meta[name=\'twitter:description\']').props().content).toBe(description)
