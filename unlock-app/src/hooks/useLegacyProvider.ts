...
import { useLegacyConnectModal } from './useLegacyConnectModal'
 ...
 */
export const useLegacyProvider = (config: any) => {
  const { setProvider, provider } = useContext(ProviderContext)
  const { openConnectModalAsync, closeConnectModal } = useLegacyConnectModal()
