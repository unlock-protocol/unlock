...
contract(
  'UnlockDiscountToken (side-chain/l2) / transferingTokens',
  (accounts) => {
    ...
    beforeEach(async () => {
      ...
      // Transfer some to Unlock
      await udt.transfer(unlock.address, web3.utils.toWei('10000', 'ether'), {
        from: minter,
      })
      ...
      await uniswapRouter.addLiquidityETH(
        ...
        minter,
        constants.MAX_UINT,
        { from: minter, value: web3.utils.toWei('4', 'ether') }
      )
    ...
    describe('grant capped by % growth', () => {
      beforeEach(async () => {
        // 10,000 UDT available for grants
        // Test goal: 10 UDT granted for the referrer (less than the gas cost equivalent of ~120 UDT)
