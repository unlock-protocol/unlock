...
contract('Lock / destroyLock', accounts => {
  ...
  before(async () => {
    ...
    tokenAddresses = [Web3Utils.padLeft(0, 40), testToken.address]
  })

  it('Test each token type', () => {
    tokenAddresses.forEach(tokenAddress => {
      describe(`Test ${tokenAddress}`, () => {
        before(async () => {
        ...
        describe('when called by the owner', () => {
          ...
          before(async () => {
            ...
            initialLockBalance = await getTokenBalance(
              lock.address,
              tokenAddress
            )
            initialOwnerBalance = await getTokenBalance(
              accounts[0],
              tokenAddress
            )
