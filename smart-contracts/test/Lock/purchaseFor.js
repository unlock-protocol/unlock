...
contract('Lock / purchaseFor', accounts => {
  ...
  it('should trigger 2 events when successful', async () => {
    ...
    const id = new BigNumber(await locks.FIRST.getTokenIdFor.call(accounts[2]))
    const duration = new BigNumber(await locks.FIRST.expirationDuration.call())

    assert.equal(tx.logs[0].event, 'Transfer')
    assert.equal(tx.logs[0].args.to, accounts[2])
    assert.equal(tx.logs[0].args.from, 0)

    assert.equal(tx.logs[1].event, 'ExpirationChanged')
    assert(new BigNumber(tx.logs[1].args._tokenId).eq(id))
    assert(new BigNumber(tx.logs[1].args._amount).eq(duration))
    assert.equal(tx.logs[1].args._timeAdded, true)
  })
