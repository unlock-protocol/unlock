...
contract('Lock / grantKeys', (accounts) => {
  ...
  describe('can grant key(s)', () => {
    describe('can grant a key for a new user', () => {
      before(async () => {
        tx = await lock.grantKey(keyOwner, validExpirationTimestamp, { from: lockOwner })
      })
    ...
    describe('can grant a key extension for an existing user', () => {
      ...
      before(async () => {
        tx = await lock.grantKey(keyOwner, extendedExpiration, { from: lockOwner })
      })
    ...
    describe('can bulk grant keys using the same expiration date', () => {
      const keyOwnerList = [accounts[3], accounts[4], accounts[5]]

      before(async () => {
        tx = await lock.grantKeys(keyOwnerList, validExpirationTimestamp, { from: lockOwner })
      })
    ...
    it('can bulk grant keys using unique expiration dates', async () => {
      ...
      const expirationDates = [validExpirationTimestamp, validExpirationTimestamp + 42]

      before(async () => {
        tx = await lock.methods['grantKeys(uint256[],uint256[])'](keyOwnerList, expirationDates, { from: lockOwner })
      })
  ...
  describe('should fail', () => {
    it('should fail to revoke a key', async () => {
      await shouldFail(
        lock.grantKey(keyOwner, 42, { from: lockOwner }),
        'ALREADY_OWNS_KEY'
      )
    })
    ...
    it('should fail to grant key to the 0 address', async () => {
      await shouldFail(
        lock.grantKey(Web3Utils.padLeft(0, 40), validExpirationTimestamp, { from: lockOwner }),
        'INVALID_ADDRESS'
      )
    })
    ...
    it('should fail to reduce the time remaining on a key', async () => {
      await shouldFail(
        lock.grantKey(keyOwner, validExpirationTimestamp - 1, { from: lockOwner }),
        'ALREADY_OWNS_KEY'
      )
    })
    ...
    it('should fail if called by someone other than the owner', async () => {
      await shouldFail(
        lock.grantKey(keyOwner, validExpirationTimestamp, { from: accounts[0] })
      )
    })
  })
})
