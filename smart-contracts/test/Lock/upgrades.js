...
describe('PublicLock upgrades', () => {
  ...
  describe('perform upgrade', async () => {
    ...
    describe('partial data migration', () => {
      beforeEach(async () => {
        // migrate only a few keys
        const [, lockOwner] = await ethers.getSigners()
        await lock.connect(lockOwner).migrateKeys(5)
      })

      it('schemaVersion has not been updated', async () => {
        assert.equal((await lock.schemaVersion()).toNumber(), 0)
      })
      ...
      it('rest of the records have NOT been updated', async () => {
        ...
        for (let i = 5; i < totalSupply; i++) {
          ...
          assert.equal(await lock.ownerOf(tokenId), buyers[i].address) // ownerOf should work regardless
          assert.equal(await lock.balanceOf(buyers[i].address), 0)
          assert.equal(await lock.getHasValidKey(buyers[i].address), false)
          assert.equal(
            (await lock.keyExpirationTimestampFor(tokenId)).toNumber(),
            0
          )
