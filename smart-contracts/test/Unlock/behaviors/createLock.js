  ...
  describe('Unlock / behaviors / createLock', function () {
    describe('lock created successfully', function () {
      ...
      it('should trigger the NewLock event', function () {
        ...
        assert(event)
        assert.equal(Web3Utils.toChecksumAddress(event.returnValues.lockOwner), Web3Utils.toChecksumAddress(accounts[0]))
      ...
      it('should have created the lock with the right address for unlock', async function () {
        ...
        assert.equal(Web3Utils.toChecksumAddress(unlockProtocol), Web3Utils.toChecksumAddress(this.unlock.address))
      })
    })
