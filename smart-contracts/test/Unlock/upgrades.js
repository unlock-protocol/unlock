...
const unlockVersions = Array(LATEST_UNLOCK_VERSION)
  .fill(0)
  .map((_, i) => i)
  // skip the missing contracts (with flattening problems to be solved)
  .filter((v) => ![2, 5].includes(v))

const publicLockVersions = unlockVersions.reduce(
  (acc, v) => ({
    ...acc,
    [v]: v,
  }),
...
// fix some corresponding Unlock versions
// as we decoupled contracts versions after v10
...
contract('Unlock / upgrades', async (accounts) => {
  ...
  before(async function copyAndBuildContract() {
    ...
    // copy all needed contract files
    for (let i = 0; i < unlockVersions.length; i++) {
      const unlockVersionNumber = unlockVersions[i]
      const publiclockVersion = publicLockVersions[unlockVersionNumber]

      // Unlock
      await fs.copy(
        require.resolve(
          `@unlock-protocol/contracts/dist/Unlock/UnlockV${unlockVersionNumber}.sol`
        ),
        path.resolve(contractsPath, `UnlockV${unlockVersionNumber}.sol`)
      )

      // Publiclock
      await fs.copy(
        require.resolve(
          `@unlock-protocol/contracts/dist/PublicLock/PublicLockV${publiclockVersion}.sol`
        ),
        path.resolve(contractsPath, `PublicLockV${unlockVersions[i]}.sol`)
      )
    }
  ...
  for (let index = 0; index < unlockVersions.length; index++) {
    const unlockVersionNumber = unlockVersions[index]
    const publicLockVersionNumber = publicLockVersions[unlockVersionNumber]

    describe(`Testing Unlock version ${unlockVersionNumber}`, () => {
      let unlock
      let publicLock
      ...
      beforeEach(async () => {
        ...
        Unlock = await ethers.getContractFactory(
          `contracts/past-versions/UnlockV${unlockVersionNumber}.sol:Unlock`
        )
        ...
        publicLock = await ethers.getContractFactory(
          `contracts/past-versions/PublicLockV${publicLockVersionNumber}.sol:PublicLock`
        )

        if (unlockVersionNumber >= 5) {
          // The lock minimal proxy was introduced with version 5
          publicLockTemplate = await publicLock.deploy()

          if (unlockVersionNumber >= 7) {
      ...
      it('PublicLock current version and latest PublicLock version have different bytecode', async () => {
          ...
          if (unlockVersionNumber >= 1) {
            ...
            if (unlockVersionNumber === 2) {
              ...
              assert.equal(
                ...
                unlockVersionNumber === 10 ? 9 : unlockVersionNumber
              )
            }
          }
        })
        ...
        describe('Purchase a key', () => {
          ...
          describe('Upgrade Unlock and PublicLock to latest version', () => {
            ...
            it('this version and latest version have different Unlock version numbers', async () => {
              const version = await unlock.unlockVersion()
              assert.notEqual(version, unlockVersionNumber)
            })
