...
UDT holders can exchange their UDT for UP on the Base network. The migration is a 1:1000 swap and can be conveniently achieved through a [migration website](https://up-swap.unlock-protocol.com/) in 4 steps:
...
The protocol aims at being governed by its implementers and users. As such, it automatically distributes UP (or UDT, depending on network) tokens on every _purchase_ of a paid membership, on applicable networks. 
...
The amount of reward tokens to be distributed on each paid transaction is calculated in real time by the Unlock contract based on several factors, including the gas consumed by the purchase transaction and the contribution to the Gross Network Product of Unlock Protocol. The Gross Network Product (GNP) is the sum of the value of all transactions sent to the network. Since memberships, subscriptions, and tickets can be priced in any ERC20, the Unlock contract consults local oracles (using Uniswap) and converts the ERC20 price into the networks's native currency to calculate the GNP.
