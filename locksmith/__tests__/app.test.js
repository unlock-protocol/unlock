...
const Lock = require('../src/sequelize')

beforeEach((done) => {
  Lock.create({ name: 'Test Lock', address: '0xbac', owner: '0xdeadbed123' }).then(() => {
    done()
  })
})
  ...
  describe('Setting lock details', () => {
    test('it saves the passed the information', (done) => {
      ...
      return request(app).post('/lock')
        ...
        .send({ 'address': 'jqa6dnp1', 'name': 'oioioi', 'expirationDuration': 2592000, 'keyPrice': '10000000000000000', 'maxNumberOfKeys': 10, 'owner': '0xAaAdEED4c0B861cB36f4cE006a9C90BA2E43fdc2' }).then(() => {
          Lock.findOne({ where: { address: 'jqa6dnp1' } }).then(record => {
            expect(record.name).toBe('oioioi')
            done()
          })
        })
    })
    ...
    test('it returns an OK status code', (done) => {
      ...
      return request(app).post('/lock')
        ...
        .send({ 'address': 'jqa6dnp1', 'name': 'oioioi', 'expirationDuration': 2592000, 'keyPrice': '10000000000000000', 'maxNumberOfKeys': 10, 'owner': '0xAaAdEED4c0B861cB36f4cE006a9C90BA2E43fdc2' }).then((response) => {
          expect(response.statusCode).toBe(200)
          done()
        })
    })
      ...
      test('it returns a 412 status code', (done) => {
        ...
        request(app).put('/lock/jqfqod74')
          ...
          .set(
            'Authorization',
            'Bearer eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0..0x0c3affb75a12cbf7e3c732aed4bf82dbe5c4baddf10ecb354585a5b7e328d25552a9092d7fd518c4ff21b0f5860fda6041a2340d2ad86b7150a0dfbf15c82c1500'
          ).send({'currentAddress':'jqfqod74','address':'0x4983D5ECDc5cc0E499c2D23BF4Ac32B982bAe53a','owner':'0xAaAdEED4c0B861cB36f4cE006a9C90BA2E43fdc2'}).then((response) => {
