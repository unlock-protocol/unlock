...
const extractSigneeFromSource = (req: Request, source: any): string | null => {
  if (!source) {
    return null
  }
  if (
...
const SignatureValidationMiddleware = {
  extractQueryParameterPayload,
  extractTypeDataSignee,
  extractPersonalSignSignee,
  extractSigneeFromSource,
  extractSignee,
  extractSigneeFromQueryParameter,
  generateSignatureEvaluator,
  handleSignaturePresent,
  validateSignee,
