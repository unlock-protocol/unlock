...
export const createHuddleRoom: RequestHandler = async (request, response) => {
  try {
    ...
    const room = await createTokenGatedHuddleRoom({
      hostWallets: [request.user!.walletAddress],
      lockAddress,
      network,
      title,
    })
    ...
    const statusCode =
      message === 'Huddle01 is not configured'
        ? 503
        : message.includes('does not support')
          ? 400
          : 502

    logger.error('Failed to create Huddle01 room', {
      error,
      requestBody: request.body,
    })
    response.status(statusCode).json({
      error: 'Failed to create Huddle01 room',
      details: message,
    })
  }
}
