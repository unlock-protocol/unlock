    ...
    if (req.signee && req.query.data) {
      ...
      return (
        ...
        ((await evaluateLockOwnership(address, req.signee, req.chain)) ||
          (await evaluateKeyOwnership(address, tokenId, req.signee, req.chain)))
      )
    }
    return false
  } catch {
...
export const updateDefaults = async (req: any, res: Response): Promise<any> => {
  ...
  if ((await evaluateLockOwnership(address, owner, req.chain)) === false) {
    ...
    const successfulUpdate = await metadataOperations.updateDefaultLockMetadata(
      {
        address,
        chain: req.chain,
        data: {
