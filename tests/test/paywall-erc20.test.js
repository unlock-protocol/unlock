...
/**
 * We are testing the behavior of the paywall application when the user has a crypto wallet with an ERC20 lock
 */
...
describe('The Unlock Ad Remover Paywall (logged in user, ERC20)', () => {
  beforeAll(async () => {
    const addresses = adblockERC20LockAddresses.map(address =>
      address.toLowerCase()
    )

    // save the lock address to pass it to the paywall
  ...
  it('should attempt a key purchase when clicking on a lock, and hide the ads', async () => {
    ...
    await checkoutIframe.waitForFunction(
      lock1 => {
        ...
        return lock && lock.dataset.testid === 'PurchaseableLock'
      },
      {},
      lockSelectors[0]('')
    )
    ...
    await checkoutIframe.waitForFunction(
      lock1 => {
        const lock = document.body.querySelector(lock1)
        return lock && lock.dataset.testid === 'ConfirmedLock'
      },
      {},
      lockSelectors[0]('')
    )
