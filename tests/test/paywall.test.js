...
const screenshotOnFail = require('../helpers/debugging').screenshotOnFail

const lockName = 'A Lock For the Paywall'
const lockKeyPrice = '0.1'
const itWithScreenshotOnFail = screenshotOnFail(page)
...
describe('The Unlock Paywall', () => {
  ...
  itWithScreenshotOnFail('should remove the blocker', async () => {
  ...
  itWithScreenshotOnFail(
    'should display the lock with the right price',
    async () => {
      expect.assertions(1)
  ...
  itWithScreenshotOnFail('scrolling is disabled', async () => {
    expect.assertions(1)
  ...
  itWithScreenshotOnFail('clicking the lock purchases a key', async () => {
    expect.assertions(1)
    const paywallIframe = page.mainFrame().childFrames()[0]
  ...
  itWithScreenshotOnFail(
    'after key purchase, unlocked flag appears',
    async () => {
      expect.assertions(0)
      const fail = () => {
        throw new Error('example fail')
      }
      await page.reload()
      await wait.forIframe()
      const paywallIframe = page.mainFrame().childFrames()[0]
